...
namespace Engage.Dnn.Employment.Admin
{
    ...
    partial class PositionListing : ModuleBase
    {
        ...
        {
            int? positionId = GetPositionId(e.RowIndex);
        ...
        private void PositionsGrid_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (string.Equals("Save", e.CommandName, StringComparison.OrdinalIgnoreCase))
            {
                if (this.Page.IsValid)
                {
                    ...
                    {
                        int? positionId = GetPositionId(rowIndex);
                        if (positionId.HasValue)
                        {
        ...
        /// <param name="args">The <see cref="System.Web.UI.WebControls.ServerValidateEventArgs"/> instance containing the event data.</param>
        protected void NewJobDescriptionRequiredValidator_ServerValidate(object source, ServerValidateEventArgs args)
        {
        ...
        private string GetJobTitle(int rowIndex)
        {
            if (this.PositionsGrid != null && this.PositionsGrid.Rows.Count > rowIndex)
            {
                ...
                TextBox txtJobTitle = row.FindControl("txtJobTitle") as TextBox;
                Debug.Assert(txtJobTitle != null);
        ...
        private string GetJobDescription(int rowIndex)
        {
            ...
            {
                GridViewRow row = this.PositionsGrid.Rows[rowIndex];
                TextEditor txtJobDescription = (TextEditor)row.FindControl("txtJobDescription");
                Debug.Assert(txtJobDescription != null);
        ...
        {
            HiddenField hdnPositionId = (HiddenField)row.FindControl("hdnPositionId");
